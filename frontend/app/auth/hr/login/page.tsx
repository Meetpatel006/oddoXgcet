...
export default function HRLogin() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const response = await api.postFormData("/auth/hr/login", formPayload)

            // Store token
            localStorage.setItem("token", response.access_token)
            localStorage.setItem("role", "hr_officer")

            // Redirect to dashboard (Assuming HR shares dashboard or has specific one. Using admin dashboard for now as base)
            ...
            <div className="w-full max-w-md">
                <div className="bg-card rounded-lg shadow-lg p-8 border border-border">
                    ...
                    <div className="text-center mb-8">
                        ...
                        <h2 className="text-2xl font-bold text-foreground">HR Login</h2>
                    ...
                    <p className="text-center text-sm text-muted-foreground">
                        New HR Officer?{" "}
                        <Link href="/auth/hr/signup" className="text-primary font-semibold hover:underline">
                            Create Account
                        </Link>
                    </p>
                </div>
                ...
                <p className="text-center text-xs text-muted-foreground mt-6">
                    For HR personnel only. Secure authentication required.
                </p>
            </div>
        </div>
    )
